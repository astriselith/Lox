...
public class Executor implements ExprVisitor<Value>, Callable {
 ...
 public Value executeFunction(
		...
		TableValue funcEnv =
			prepareFunctionExecution(closure, function.parameters, function.hasVarargs, arguments);
 ...
 public Value visitVarDeclExpr(VarDeclExpr expr) {
		...
		for (Expr decor : expr.decorators) {
			...
			if (decorResult.isReturn() || decorResult.isThrow())
				return decorResult;
 ...
 public Value visitDecorExpr(DecorExpr expr) {
		...
		if (expr.arguments != null) {
			for (Expr arg : expr.arguments) {
				...
				if (argVal.isReturn() || argVal.isThrow())
					return argVal;
				args.add(argVal);
			}
		}
 ...
 public Value visitBinaryExpr(BinaryExpr expr) {
		...
		if (op.equals("??")) {
			...
			if (right.isReturn() || right.isThrow())
				return right;
			return right;
		}
			...
			if (op.equals("++") || op.equals("--")) {
				...
				try {
					if (target instanceof VarExpr) {
						...
						if (index.isReturn() || index.isThrow())
							return index;
