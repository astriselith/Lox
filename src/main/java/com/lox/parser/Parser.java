...
public class Parser {
	...
	private static final String ERR_BLOCK_TABLE = "Bloco tabela só pode conter declarações (var, fun, ou campo nome: valor)";
	...
	private Position between(Expr start, Expr end) {
		return Position.between(start.getPosition(), end.getPosition());
	}

	private Position between(Expr start, Position end) {
		return Position.between(start.getPosition(), end);
	}

	private Position between(Position start, Expr end) {
		return Position.between(start, end.getPosition());
	}

	private Position between(Token start, Expr end) {
		return Position.between(pos(start), end.getPosition());
	}

	private Position between(Expr start, Token end) {
		return Position.between(start.getPosition(), pos(end));
	}
	...
	private Expr object() {
		...
		while (!stream.isAtEnd() && !stream.check(RBRACE)) {
			...
			if (stream.check(IDENTIFIER) && stream.checkNext(COLON)) {
				Token name = stream.advance();
				stream.advance();
				Expr value = expr();
				VarDeclExpr field = new VarDeclExpr(name.lexeme, value, decorators, between(name, value));
				declarations.add(field);
				semicolon();
				continue;
			}
			if (stream.check(VAR)) {
				declarations.add(varDecl(decorators));
				continue;
			}
			if (stream.check(FUN)) {
			...
			if (!decorators.isEmpty()) {
				throw error(ERR_DECOR_VAR, stream.peek());
			}
	...
	private Expr lambdaDecl() {
		...
		if (!hasParens && !stream.check(ARROW)) {
			throw error(ERR_LAMBDA_ARROW);
		}
		boolean hasArrow = stream.match(ARROW);
		FunDeclExpr prev = currentFunction;
		FunDeclExpr lambda = new FunDeclExpr(null, params.getFirst(), new ArrayList<>(), params.getSecond(), null);
	...
	private Expr call() {
		...
		while (true) {
			...
			if (stream.check(LPAREN)) {
				...
				expr = new CallExpr(expr, args, between(expr.getPosition(), !args.isEmpty() ? between(args) : expr.getPosition()));
				continue;
			}
