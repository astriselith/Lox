...
public class Parser {
 ...
 private static final String ERR_INVALID_STMT_EXPR =
		"Expressão não pode ser usada como statement. Apenas atribuições, chamadas de função e "
		+ "incrementos/decrementos são permitidos";
 private static final String ERR_INVALID_UNARY_STMT =
		"Operador unário '%s' não pode ser usado como statement";
 ...
 private static final String ERR_ARROW_RETURN =
		"Return statement não permitido em arrow function. O valor é implícito";
 private static final String ERR_ARROW_BREAK_CONTINUE =
		"Break/continue não permitido em arrow function";
 ...
 private void semicolon() {
		...
		if (!stream.match(SEMICOLON))
			throw error(ERR_EXPECTED_SEMICOLON);
 }

 private List<Expr> body() {
		if (!stream.match(LBRACE))
			throw error(ERR_LBRACE);
 ...
 private Expr varDecl(List<Expr> decorators) {
		...
		return new VarDeclExpr(name.lexeme, value, decorators, between(start, stream.previous()));
 }
 ...
 private Expr funDecl() {
		...
		if (!stream.check(LPAREN))
			throw error(ERR_LPAREN);

		Pair<List<String>, Boolean> params = parameters();
		FunDeclExpr prev = currentFunction;

		FunDeclExpr f = new FunDeclExpr(name, params != null ? params.getFirst() : null, null,
			params != null && params.getSecond(), null);
		...
		if (stream.match(ARROW)) {
			...
			f.setBody(bodyList);
			f.setPosition(
				between(start.getPosition(), bodyList.isEmpty() ? start.getPosition() : between(bodyList)));
		} else {
 ...
 private Expr lambdaDecl() {
		...
		FunDeclExpr prev = currentFunction;
		FunDeclExpr lambda =
			new FunDeclExpr(null, params.getFirst(), new ArrayList<>(), params.getSecond(), null);

		currentFunction = lambda;
		boolean hasArrow = stream.match(ARROW);

		if (!hasParens && !hasArrow)
			throw error(ERR_LAMBDA_ARROW);
 ...
 private Expr breakStmt() {
		...
		if (loopDepth == 0)
			throw error(ERR_BREAK_OUTSIDE, t);
		semicolon();
 ...
 private Expr continueStmt() {
		Token t = stream.advance();
		if (loopDepth == 0)
			throw error(ERR_CONTINUE_OUTSIDE, t);
		semicolon();
 ...
 private Expr exprStmt() {
		...
		if (!(e instanceof AssignExpr || e instanceof SetExpr || e instanceof IndexSetExpr
							|| e instanceof CallExpr || e instanceof UnaryExpr)) {
		...
		if (op.isAssignment()) {
			...
			if (expr instanceof VarExpr) {
				...
				if (op.type != EQ) {
					...
					Expr binary = new BinaryExpr(expr, binaryOp, value, between(op, value.getPosition()));
					return new SetExpr(
						get.object, get.name, binary, between(expr.getPosition(), value.getPosition()));
				}

				return new SetExpr(
					get.object, get.name, value, between(expr.getPosition(), value.getPosition()));
				...
				if (op.type != EQ) {
					String binaryOp = op.lexeme.substring(0, op.lexeme.length() - 1);
					Expr binary = new BinaryExpr(expr, binaryOp, value, between(op, value.getPosition()));
					return new IndexSetExpr(
						idx.array, idx.index, binary, between(expr.getPosition(), value.getPosition()));
				}

				return new IndexSetExpr(
					idx.array, idx.index, value, between(expr.getPosition(), value.getPosition()));
			}
 ...
 private Expr logicalOr() {
		...
		while (stream.match(OR_OR)) {
			...
			expr = new LogicalExpr(
				expr, op.lexeme, logicalAnd(), between(expr.getPosition(), stream.previous().getPosition()));
		}
 ...
 private Expr logicalAnd() {
		...
		while (stream.match(AND_AND)) {
			Token op = stream.previous();
			expr = new LogicalExpr(
				expr, op.lexeme, coalesce(), between(expr.getPosition(), stream.previous().getPosition()));
		}
 ...
 private Expr ternary() {
		...
		if (stream.match(QUESTION)) {
			...
			if (!stream.match(COLON))
				throw error(ERR_COLON);
			Expr elseExpr = expr();
			return new TernaryExpr(
				expr, thenExpr, elseExpr, between(expr.getPosition(), elseExpr.getPosition()));
		}
 ...
 private Expr equality() {
		...
		while (stream.matchAny(EQ_EQ, NOT_EQ)) {
			Token op = stream.previous();
			expr = new BinaryExpr(
				expr, op.lexeme, comparison(), between(expr.getPosition(), stream.previous().getPosition()));
		}
 ...
 private Expr comparison() {
		...
		while (stream.matchAny(LT, GT, LTE, GTE)) {
			Token op = stream.previous();
			expr = new BinaryExpr(
				expr, op.lexeme, term(), between(expr.getPosition(), stream.previous().getPosition()));
		}
 ...
 private Expr term() {
		...
		while (stream.matchAny(PLUS, MINUS)) {
			Token op = stream.previous();
			expr = new BinaryExpr(
				expr, op.lexeme, factor(), between(expr.getPosition(), stream.previous().getPosition()));
		}
 ...
 private Expr factor() {
		...
		while (stream.matchAny(STAR, SLASH, PERCENT)) {
			Token op = stream.previous();
			expr = new BinaryExpr(
				expr, op.lexeme, unary(), between(expr.getPosition(), stream.previous().getPosition()));
		}
 ...
 private Expr call() {
		...
		while (true) {
			...
			if (stream.check(LPAREN)) {
				...
				expr = new CallExpr(
					expr, args, between(expr.getPosition(), !args.isEmpty() ? between(args) : expr.getPosition()));
				continue;
			}
 ...
 private Pair<List<String>, Boolean> parameters() {
		...
		if (!stream.check(RPAREN)) {
			do {
				...
				if (!stream.check(IDENTIFIER))
					throw error(ERR_PARAM);

				Token p = stream.advance();
				params.add(p.lexeme);

				if (isVararg) {
					if (!stream.checkAny(RPAREN, COMMA))
						throw error(ERR_VARARGS_LAST, p);
